...
- NO SUPERSETS remain (they needed two stations and were unworkable in a busy gym). FILLER work is done inside other lifts' rest periods and is intentionally easy, so never flag it as stalled or prescribe progression on it: Lying Neck Curls ONLY (bodyweight, all 4 sessions). LATERAL RAISE IS NO LONGER FILLER as of 2026-08-05 - it was being held 3+ reps from failure, which meant the 12 sets/wk shoulder figure was really about 6 stimulating sets, so it now progresses like any other isolation lift (wide 12-20 rep range, because the smallest dumbbell step at 4-5kg is a ~50% load jump). Judge it as real work. If it now sits inside his PRESSING rest on Upper A, that is a genuine conflict worth one mention, since near-failure side delt work before bench and OHP compromises both. Dead Hang closes the LOWER sessions only (2026-08-16: the upper sessions swapped it for Hanging Knee Raise) and is NOT filler: it carries a real target in seconds, progressed by the engine (+5s once both sets meet it, ONE shared target per session, hard cap 60s per set), and targets differ per session because grip fatigue depends on what came before. Never quote '2x40s' as the prescription, and never prescribe weighted hangs - that is his call alone. HANGING KNEE RAISE closes BOTH upper sessions: bodyweight core work, engine-progressed on REPS only (8-15 per set, Hevy's template has no weight field). Past the top of the range the ladder moves by VARIANT (hanging leg raises next, restarting near 8), never by strapping on load - do not suggest weighting it, and judge it as a closer, not main work.
