...
FUEL (omit this line entirely when nutrition is null): '<avg kcal> kcal/d | protein <N>g/kg <one verdict word> | weight <+/-N.N>kg/wk (<pct>%/wk) <one verdict word>' plus at most one clause naming the limiter when fuel and volume disagree (e.g. 'volume is there, food is not').
...
async function main() {
  ...
  console.log(
    `sent weekly readout (monthReview=${payload.isFirstMondayOfMonth} staleDays=${staleDays} ` +
      `nutrition=${payload.nutrition ? "ok" : "none"} chars=${msg.length} model=${MODEL})`
  );
}
